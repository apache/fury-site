...
这与序列化一行数据或 JSON 文档不同。对象序列化可以保留具体对象类型和对象身份，使读取端能够
...
各 Fory 实现的线程安全方式不同。有些实现提供线程安全包装器或对象池，另一些要求每个线程或任务
...
静态已知字段可以直接使用其声明类型。动态字段还会携带具体类型，用于接口、抽象类、
Trait Object、宽泛对象类型或异构值。动态类型更灵活，但所有可能的具体类型都必须完成注册，
并且受到所选模式支持。

xlang 模式要求所有通信方协调一致的可移植类型标识和类型映射；原生模式可以使用特定于实现的
标识和语言特定的类型。可移植规则请参阅 [Xlang 序列化](xlang.md)，具体注册 API 请参阅各语言的类型注册页面。
...
有些 Fory 实现同时使用全局设置和字段级元数据，准确行为请参阅对应语言的“引用”或“基础序列化”页面。
...
候选类型。同一 Fory 实现家族内的数据可以通过原生模式使用更多语言特有的 Class、Trait 或钩子行为。
...
- [Xlang 序列化](xlang.md)是默认模式；通信方使用不同 Fory 实现家族或需要可移植契约时必须使用它。
- [原生序列化](native.md)用于同一 Fory 实现家族内受支持的场景，适合需要原生类型或原生行为的数据。
