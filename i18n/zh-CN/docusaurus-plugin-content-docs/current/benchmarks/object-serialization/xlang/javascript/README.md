...
计时的序列化器循环使用各序列化器原生的类型化值。Fory 接收其模式所使用的预规范化 Fory 值，protobuf 接收预先构建的 protobuf 形态值，JSON 接收基准测试所用的 JavaScript 对象。protobuf 的计时不包括 `toProto`、`fromProto`、`protobufjs.create` 或 `toObject` 的转换开销。

## 基准测试图

图中展示吞吐量（ops/sec）；数值越高越好。
...
| CPU 核心数（逻辑）         | 12                       |
| 内存总量（GB）             | 48.0                     |
| 基准测试日期               | 2026-05-08T08:07:36.073Z |
| CPU 核心数（来自基准测试） | 12                       |
| Node.js                    | v22.20.0                 |
