...
Swift 引用跟踪由 `Config.trackRef` 控制。
...
`trackRef` 会保留引用图，但不会改变 ARC 所有权。为避免内存泄漏，请在循环中至少一条边上
使用 `weak`。
...
- 值类型（`struct`、基本类型值）不具有对象标识语义
- `trackRef` 控制序列化图的对象标识，而不是 ARC 内存所有权
- 对纯值载荷使用 `trackRef=false` 以降低开销
