...
有关 fory-json、Jackson 和 Gson 的测试结果，请参阅 [Java JSON 基准测试报告](../../../json/java/README.md)。
...
MEDIA_CONTENT 是来自 [jvm-serializers](https://github.com/eishay/jvm-serializers/blob/master/tpc/src/data/media/MediaContent.java) 的类。

### Sample

SAMPLE 是来自 [Kryo 基准测试](https://github.com/EsotericSoftware/kryo/blob/master/benchmarks/src/main/java/com/esotericsoftware/kryo/benchmarks/data/Sample.java) 的类。
...
读取端和写入端使用相同的类 Schema。此模式不使用类的向前/向后兼容能力。

![Java 堆内同 Schema 序列化](java_heap_serialize_consistent.png)

#### Java 兼容序列化

读取端和写入端可以使用不同的类 Schema。此模式支持类的向前/向后兼容。

![Java 堆内 Schema 兼容序列化](java_heap_serialize_compatible.png)

#### Java 同 Schema 反序列化

读取端和写入端使用相同的类 Schema。此模式不使用类的向前/向后兼容能力。

![Java 堆内同 Schema 反序列化](java_heap_deserialize_consistent.png)

#### Java 兼容反序列化

读取端和写入端可以使用不同的类 Schema。此模式支持类的向前/向后兼容。

![Java 堆内 Schema 兼容反序列化](java_heap_deserialize_compatible.png)
...
![Java 堆外同 Schema 序列化](java_offheap_serialize_consistent.png)

#### Java 兼容序列化

读取端和写入端可以使用不同的类 Schema。此模式支持类的向前/向后兼容。

![Java 堆外 Schema 兼容序列化](java_offheap_serialize_compatible.png)

#### Java 同 Schema 反序列化

读取端和写入端使用相同的类 Schema。此模式不使用类的向前/向后兼容能力。

![Java 堆外同 Schema 反序列化](java_offheap_deserialize_consistent.png)

#### Java 兼容反序列化

读取端和写入端可以使用不同的类 Schema。此模式支持类的向前/向后兼容。

![Java 堆外 Schema 兼容反序列化](java_offheap_deserialize_compatible.png)

### 零拷贝序列化

请注意，零拷贝序列化只避免序列化过程中的数据复制；如果将数据发送到其他机器，仍然可能发生复制。

但是，如果在同一节点的进程之间通过共享内存序列化数据，并且数据在序列化前已位于堆外内存中，那么其他进程无需任何复制即可读取此缓冲区。

#### Java 零拷贝序列化至堆内缓冲区

![Java 零拷贝序列化](java_zero_copy_serialize.png)

#### Java 零拷贝序列化至直接缓冲区

![Java 零拷贝反序列化](java_zero_copy_deserialize.png)
