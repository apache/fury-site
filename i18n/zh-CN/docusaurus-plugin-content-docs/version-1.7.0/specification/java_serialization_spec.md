...
- 如果字段具有显式非负 `@ForyField(id = ...)`，该数值 ID 即为字段标识符。
- 否则，将转换为 snake_case 的 Java 字段名用作字段标识符。
- 负数注解值不是有效字段 ID。注解默认值 `-1` 表示没有显式 ID，选择标识符时会忽略它。
...
对于 tag ID 编码，位 `4..6` 存储数值字段 ID，前提是该 ID 小于 `7`。
如果值为 `7`，则再读取一个 `varuint32` 并加到 `7` 上。字段 ID 必须为非负数。
同一个 TypeDef 中不能出现重复字段 ID。
