...
Fory 不会检查被引用包中的私有成员。精确私有映射是由应用负责的包 ABI 声明。请将包版本与声明一起固定并测试。如果映射的私有字段发生变化，生成的精确访问器会以 CLR 缺少字段错误失败；Fory 不会回退到反射或其他成员。被忽略的私有字段没有生成的访问器，因此请对照固定的包构建验证该存储声明。
...
通过 Fory C# 已经支持的具体载体类型，可以支持外部子项：
