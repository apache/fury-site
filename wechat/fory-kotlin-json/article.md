...
可空性检查也覆盖容器元素和嵌套泛型中的类型。如果需要使用次构造函数、通过 `JsonCreator` 指定构造函数或工厂方法，或处理类体中声明的属性，可参阅 [Kotlin 指南](https://fory.apache.org/zh-CN/docs/json/kotlin#immutable-classes-and-compiler-defaults)。
...
如果可空值类的表示会让 JSON null 产生歧义，Fory 会拒绝自动映射，需要使用带标签的自定义编解码器；具体规则见[类型支持表](https://fory.apache.org/zh-CN/docs/json/kotlin#supported-kotlin-types)。
...
这个注解把 Kotlin 中的 `id` 对应到 JSON 中的 `user_id`。属性命名、格式设置、Mixin 和自定义编解码器都沿用 Fory 的注解机制。支持哪些 Kotlin 注解目标，可参阅[注解指南](https://fory.apache.org/zh-CN/docs/json/annotations#kotlin-use-site-targets)。

## Fory 如何高效映射 Kotlin 模型
...
这些编解码器沿用 Fory 已有的数字编码和文本处理实现，通过复用缓冲区减少临时分配，并支持直接读写 UTF-8 字节。Kotlin 模块负责类型规则和对象构造，JSON 引擎负责解析与输出，整个过程不需要先构建一棵 JSON 树。这些底层优化的具体实现，可参阅 [Java JSON 文章](https://fory.apache.org/zh-CN/blog/fory_json_fastest_java_json_framework)。
...
![Kotlin MediaContent String 序列化与反序列化吞吐量](../../static/img/blog/fory-kotlin-json/string_throughput.png)

![Kotlin MediaContent UTF-8 字节序列化与反序列化吞吐量](../../static/img/blog/fory-kotlin-json/utf8_bytes_throughput.png)

### Users 和 Clients：约 1 MB 的大文档
...
![Kotlin Users String 序列化与反序列化吞吐量](../../static/img/blog/fory-kotlin-json/users_string_throughput.png)

![Kotlin Users UTF-8 字节序列化与反序列化吞吐量](../../static/img/blog/fory-kotlin-json/users_utf8_bytes_throughput.png)
...
![Kotlin Clients String 序列化与反序列化吞吐量](../../static/img/blog/fory-kotlin-json/clients_string_throughput.png)

![Kotlin Clients UTF-8 字节序列化与反序列化吞吐量](../../static/img/blog/fory-kotlin-json/clients_utf8_bytes_throughput.png)
...
## JVM、Android 与 GraalVM

该模块可用于 Kotlin/JVM 项目，支持 Android 和 GraalVM Native Image，且不依赖 `kotlin-reflect`。兼容性说明见[安装指南](https://fory.apache.org/zh-CN/docs/json/kotlin#installation)。使用 JDK 25 及更高版本时，建议按[运行时配置指南](https://fory.apache.org/zh-CN/docs/json/getting-started)开放 `java.lang.invoke` 包的访问权限。
...
在 GraalVM Native Image 中，通过 `ForyJsonProvider` 注册 Kotlin 模块，并为应用可访问到的模型配置代码生成。这两种环境的配置方法见[平台指南](https://fory.apache.org/zh-CN/docs/json/kotlin#graalvm-and-android)。该模块不支持 Kotlin/Native、Kotlin/JS 和 Kotlin/Wasm。

## 延伸阅读

完整的类型映射与配置说明见 [Kotlin JSON 指南](https://fory.apache.org/zh-CN/docs/json/kotlin)。需要自定义 JSON 表示时，可参考[自定义编解码器](https://fory.apache.org/zh-CN/docs/json/custom-codecs)；输入限制的配置见[安全指南](https://fory.apache.org/zh-CN/docs/json/security)。源码和贡献说明见 [apache/fory](https://github.com/apache/fory)。

本文整理自 [Apache Fory 中文博客](https://fory.apache.org/zh-CN/blog/fory_kotlin_json)。
